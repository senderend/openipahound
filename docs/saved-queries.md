...
Saved queries are not graph data and they are not removed when OpenIPAHound graph
data or the OpenIPAHound schema is deleted. Remove them separately from
BloodHound's Saved Queries UI if you want a clean UI before reinstalling or
reimporting.
